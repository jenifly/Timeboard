...
public class PickerView extends View
{
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        mViewHeight = (int)(MARGIN_ALPHA * 5 * mMinTextSize);
        isInit = true;
    ...
    private void drawData(Canvas canvas)
    {
        ...
        mPaint.setColor( Cache.theme.getBGCOLOR_DARK());
        ...
        String str = mDataList.get(mCurrentSelected);
        canvas.drawText(str, x-size/4.5f, baseline, mPaint);
        mPaint.setTextSize(size/2);
        canvas.drawText(pvName, x+ mPaint.measureText(str) + 10, baseline-size/2, mPaint);
        mPaint.setColor(mColorText);
        mPaint.setAlpha(12);
        int padding = mViewHeight/10 + 8;
        int centerY = mViewHeight/2;
        Path path = new Path();
        path.moveTo(0,centerY-padding);
        path.lineTo(mViewWidth,centerY-padding);
        path.lineTo(mViewWidth,centerY + padding);
        path.lineTo(0,centerY + padding);
        path.close();
    ...
    private void drawOtherText(Canvas canvas, int position, int type)
    {
        ...
        float scale = parabola(mViewHeight / 4.1f, d);
        float size = (mMaxTextSize - mMinTextSize) * scale + mMinTextSize;
        mPaint.setTextSize(size);
        mPaint.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
