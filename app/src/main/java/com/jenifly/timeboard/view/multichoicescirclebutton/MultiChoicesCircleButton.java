package com.jenifly.timeboard.view.multichoicescirclebutton;
...
import com.jenifly.timeboard.utils.DrawableUtils;
...
public class MultiChoicesCircleButton extends View {
    ...
    public MultiChoicesCircleButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mExpandRadius = typedArray.getDimension(R.styleable.MultiChoicesCircleButton_mccb_expandRadius, dp2px(140));
        mText = typedArray.getString(R.styleable.MultiChoicesCircleButton_mccb_text);
        mTextSize = typedArray.getDimension(R.styleable.MultiChoicesCircleButton_mccb_textSize, sp2px(28));
        ...
        mItemRadius = typedArray.getDimension(R.styleable.MultiChoicesCircleButton_mccb_itemRadius, dp2px(26));
    ...
    private boolean actionDownInCircle(float x, float y) {
        ...
        return distance <=  Math.pow(currentRadius * 3 / 2, 2);
    }
    ...
    private int getSelectedItemIndex(float x, float y) {
        if (!mItems.isEmpty()) {
            for (int i = 0; i < mItems.size(); i++) {
                ...
                if (distance < Math.pow(mItemRadius,2)) {
                    return i;
                }
            }
        }
    ...
    public Point getHeartPoint(float angle,float radiu) {
        float t = (float) (angle / Math.PI);
        float x = (float) (radiu * (16 * Math.pow(Math.sin(t), 3)));
        float y = (float) ((-radiu-1) * (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t)));
        return new Point((int)mCircleCentreX + (int)x, (int)mCircleCentreY + (int)y- (int)radiu * 8);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (text != null && text.length() != 0) {
            ...
            final float baseLineY = mCircleCentreY - radius - textHeight - dp2px(24);
        ...
        if (!mItems.isEmpty()) {
            ...
            for (int i = 0; i < mItems.size(); i++) {
                ...
                mPaint.setAlpha(mHoverItemIndex == i ? 255 : 140);
                float offsetX = (float) (mItemDistanceToCentre * Math.cos(Math.PI * item.angle / 180));
                float offsetY = 0;
                switch (i){
                    case 0:
                ...
                if (item.icon != null) {
                    Drawable drawable = DrawableUtils.TintDrawable(item.icon,  theme.getBGCOLOR_DARK());
                    float iconRadius = mItemRadius * 3 / 4 * mCurrentExpandProgress;
